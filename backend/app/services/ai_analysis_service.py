...
- Do NOT tell the reader to buy, sell, switch, hold, exit, add to, or stop anything. Do NOT recommend a specific fund to move to. Do NOT suggest how much to invest or when.
- Describe what the record shows and what it does not. Where the evidence is mixed, say it is mixed rather than resolving it.
...
"TRADER RULES (always respect these): stop loss never wider than 5-6% of entry price; ideal trade risk is 3-4% of position value/invested amount, not 3-4% of total account equity. Total account risk on any single idea must never exceed 1% of total equity: position size must satisfy (entry - stop) * quantity <= 1% of equity. If price goes against the trade, cut half early and exit fully at the stop; entries should be near the 10/21 EMA on pullbacks, or at the pivot on orderly 2-week-plus bases.\n\n"
...
"THE STUDENT'S STATED RULES - grade them against these: stop loss never wider than 5-6% of entry price; ideal loss/risk is 3-4% of position value/invested amount, not 3-4% of total account equity. Total account risk on any single idea must never exceed 1% of starting equity: position size must satisfy (entry - stop) * quantity <= 1% of equity. Do not call 3-4% of position value an account-risk breach unless the rupee risk also exceeds 1% of equity. Explicitly flag every trade where stop distance was too wide, account risk exceeded 1%, the actual position loss broke the rule, or they failed to cut early.\n\n"
