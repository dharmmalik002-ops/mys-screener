...
72. **THE FINAL PRODUCT IS THE TWO SURVIVORS BLENDED, AND 5 OF 691 REAL FUNDS BEAT IT ON BOTH AXES.** Only two components survived this project: regime timing (when to be in the market) and the defensive breaker (which cells to stand down). `combined.py` holds both as 50/50 sleeves, **no rebalancing**, and `scripts/combined_product.py` measures the result. They are complementary — over the held-out window the blend's Sharpe is **1.20** against the book's 0.90 and timing's 0.97, with a shallower drawdown than either — which is ordinary diversification, not a new edge.

    Against 691 real Indian equity funds over the **exact same three years** (the window is cut to match precisely, because a 3.8y-vs-3y mismatch was false positive #2 here):
    ...
    **Both halves travel together or the number is a lie.** The blend **earns less than the median fund** and sits in the **18.5th percentile on return alone** — anyone who cares only about return should prefer most of these funds. What it does is earn below-median money at a quarter of the drawdown, which is why only five funds dominate it on both measures at once. `funds_dominating` is the deciding measure precisely because a fund can win either axis by losing the other. And two caveats from `benchmark.CAVEATS` are load-bearing: the bot may sit in cash and a fund may not (a large structural advantage in a falling market, and not skill), and the bot's returns are simulated while the funds' are realised money net of fees actually charged.
