...
21. **The Space Only Gets EOD Data From `daily-bhavcopy.yml` Step 9:** `deploy.yml` is skipped for those commits (they carry `[skip ci]`, and a `GITHUB_TOKEN` push cannot trigger a workflow anyway), so the bhavcopy job pushes to the Space itself. When that step fails the repo still has the day's patch and the Space silently does not — check `/api/bhavcopy/status` against the newest `data:` commit. The manual recovery is the `workflow_dispatch` on **Deploy to HuggingFace**, which uses the same secret and is known to work.
