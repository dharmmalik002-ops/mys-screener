...
125. **A Chart Cache Checked Against the Calendar Is No Cache At All:** `readCachedChart` expires any chart whose newest bar predates the session the app is showing. That session came from `breadth_today.date`, which was *today's IST calendar date* — so on weekends, holidays and every weekday until the close, every cached and prewarmed chart read as stale. The prewarm then re-fetched its whole 20-chart window on every step, filled the backend's 3 chart slots, and the chart the user actually clicked queued behind them: 5-10 s per "next stock". `breadth_today.date` is now the newest `history_session_date`, and the frontend also clamps it to the newest bar a live chart has served since that session was announced. Related: user chart opens join in-flight prewarms through `fetchChartShared` (an open used to fire the same fetch twice), and the API is gzipped (a 1D chart is ~68 KB raw).

126. **Market Breadth Comes From NSE's Bhavcopy, Never From Yahoo or Our Caches:** audited day by day against NSE's official files (2024-09 to 2026-09) and four defects were fixed. (a) The Home 10-session chart was counted off the per-stock chart cache — gitignored, so **4 symbols** on the Space (0% advancing one day, 75% the next, beside a 49% card). It now reads `nse_breadth_history.json`: every NSE mainboard share (EQ/BE/BZ, `INE` ISIN), one row per session NSE published, written by `_update_exchange_breadth` in the daily job and committed. (b) The XP inputs used the BSE file overlaid with **yfinance** records, whose "previous close" is two sessions old whenever Yahoo drops a day — 2026-09-18 read 230 stocks up 4.5% against NSE's 162. Breadth now reads NSE's file first, BSE's own file second, and never the overlay. (c) The XP EMAs were never restated for splits/bonuses, so a split stock read "below its EMA" for weeks; the state is now `[ema10, ema20, last_close]` and rescales when the exchange's previous close disagrees by >3%. (d) The yfinance fallback returned the newest bar on or before the target date without checking its date, so holidays got rows (2026-09-14 Ganesh Chaturthi, plus 15 Jan, 1 May, 28 May, 26 Jun) — it now requires the bar to be the target session. Rebuilt with `backfill_xp_breadth.py --cache-dir` (NSE-first). Two things to know: 2025-02-01 was a real **Saturday** session (Budget day), so "weekday" is not a trading calendar; and the XP's fit to the author's published EM moved from MAE 0.69 to 1.26 — the old figure relied on a missing 2026-06-09 (a 206-advancer day) that dragged 10-12 June down toward the author's 10.5. The inputs are now exact; the model simply reads that week higher than EM does.
